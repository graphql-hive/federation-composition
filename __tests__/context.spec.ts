...
testVersions((api, version) => {
  const compose: typeof api.composeServices = (services, __internal) =>
    api.composeServices(services, {
      enableContextDirectives: true,
      ...__internal,
    });
  const subgraph = (
    name: string,
  ...
  api.runIf("guild", () => {
    test("rejects the context directives if the feature flag is off", () => {
        ...
        subgraph(
          ...
          graphql`
            ...
            type Discount @key(fields: "id") {
              id: ID!
              rate(
                segment: String @fromContext(field: "$customerCtx { segment }")
              ): Int!
            }
      ...
      expect(result.errors).toContainEqual(
        expect.objectContaining({
          message: "[orders] @context directive is not yet supported.",
          extensions: expect.objectContaining({
      ...
      expect(result.errors).toContainEqual(
        expect.objectContaining({
          message: "[orders] @fromContext directive is not yet supported.",
          extensions: expect.objectContaining({
            code: "UNSUPPORTED_FEATURE",
          }),
        }),
        ...
        subgraph(
          ...
          graphql`
            ...
            type ReviewScorer @key(fields: "id") {
              id: ID!
              score(
                language: String @fromContext(field: "$reviewCtx { language }")
              ): Int!
            }
        ...
        const result = compose([
          subgraph(
            ...
            graphql`
              ...
              type ReviewScorer @key(fields: "id") {
                id: ID!
                score(sentiment: String @fromContext(field: "sentiment")): Int!
              }
      ...
      test("accepts equivalent selections from multiple concrete providers", () => {
        const result = compose([
          subgraph(
            "pricing",
            graphql`
              union PriceSource = RetailAccount | WholesaleAccount | GuestQuote
              ...
              type RetailAccount @key(fields: "id") @context(name: "priceCtx") {
                id: ID!
                currency: String!
              }
          ...
          subgraph(
            ...
            graphql`
              ...
              type QuoteEngine @key(fields: "id") {
                id: ID!
                calculate(
                  currency: String @fromContext(field: "$priceCtx { currency }")
                ): Int!
              }
        ...
        services: [
          subgraph(
            ...
            graphql`
              ...
              type Shelf @key(fields: "shelfCode") @context(name: "shelfCtx") {
                shelfCode: String!
                items: [StockItem!]!
                robot: PickRobot!
              }
      ...
      const result = compose([
        ...
        subgraph(
          ...
          graphql`
            ...
            type Wallet @key(fields: "id") {
              id: ID!
              measure(age: Int @fromContext(field: "$memberCtx { age }")): Int!
            }
          `,
        ),
      ]);
          ...
          subgraph(
            ...
            graphql`
              ...
              type Wallet @key(fields: "id") {
                ...
                apply(
                  plan: String @fromContext(field: "$memberCtx { plan }")
                ): Int! @requiresScopes(scopes: [["member:read", "plan:read"]])
              }
