...
const nodes: {
  ...
  FragmentSpread(node) {
    return join(
      ["...", print(node.name), join(node.directives?.map(print), " ")],
      " ",
    );
  },
  InlineFragment(node) {
  ...
  ScalarTypeDefinition(node) {
    return (
      ...
      join(
        ["scalar", print(node.name), join(node.directives?.map(print), " ")],
        " ",
      )
    );
  },
...
function join(
  maybeArray: ReadonlyArray<string | undefined> | undefined,
  separator = "",
): string {
